...
export function normalizeParameter(
  ...
  let schema;
  if (is$Ref(parameter)) {
    schema = dereferenceSchema(ajv, parameter['$ref' as keyof OpenAPIV3.ParameterObject]);
  } else if (parameter?.schema?.['$ref' as keyof (OpenAPIV3.ReferenceObject | OpenAPIV3.SchemaObject)]) {
    schema = dereferenceSchema(ajv, parameter.schema['$ref'  as keyof (OpenAPIV3.ReferenceObject | OpenAPIV3.SchemaObject)]);
  } else {
  ...
  return { name, schema: schema as OpenAPIV3.SchemaObject };
}
...
export function dereferenceSchema(ajv: Ajv, ref: string) {
  ...
  if (derefSchema?.['$ref' as keyof AnyValidateFunction]) {
